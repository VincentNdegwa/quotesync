...
import type { GroupedActivity, InvoiceActivity } from '@/types';
...
const colorForType = (type: string): string => {
    return getInvoiceActivityType(type)?.color ?? 'text-muted-foreground bg-muted';
};
...
const grouped = computed((): GroupedActivity[] => {
    ...
    while (i < sorted.length) {
        ...
        if (isMinorEvent(current.type)) {
            const run: InvoiceActivity[] = [current];
            let j = i + 1;
