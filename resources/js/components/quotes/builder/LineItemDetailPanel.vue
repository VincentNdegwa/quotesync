...
const emit = defineEmits<{
    ...
    (
        e: 'toggle-tax',
        payload: { tax: BuilderTaxOption; enabled: boolean },
    ): void;
}>();
...
const currentCatalog = computed(() => {
    ...
    return (
        props.catalogItems.find(
            (entry) => entry.id === props.lineItem?.catalog_item_id,
        ) ?? null
    );
});
...
const margin = computed(() => {
    ...
    const cost =
        Number(props.lineItem.cost_price || 0) *
        Number(props.lineItem.quantity || 0);
    const revenue =
        Number(props.lineItem.unit_price || 0) *
        Number(props.lineItem.quantity || 0);
...
const selectedTaxIds = computed<string[]>({
    ...
    set: (nextValues) => {
        const currentIds = new Set(
            props.lineItem?.taxes.map((tax) => String(tax.tax_id)) ?? [],
        );
        ...
        <div class="custom-scrollbar space-y-3 overflow-y-auto p-3">
            ...
            <div class="grid gap-2 text-xs text-muted-foreground">
                ...
                <p v-if="margin">
                    ...
                    {{ fmt(margin.profit, props.currency) }}
                </p>
            </div>
                ...
                <CatalogSearchPopover
                    ...
                    @select="
                        (catalogItem: BuilderCatalogItem) =>
                            emit('select-catalog-item', catalogItem)
                    "
                    @add-custom="
                        () => {
                            emit('update-field', {
            ...
            <div class="grid gap-2">
                ...
                <Input
                    ...
                    @update:model-value="
                        (value: string | number) =>
                            emit('update-field', { field: 'name', value })
                    "
                />
            </div>
            <div class="grid gap-2">
