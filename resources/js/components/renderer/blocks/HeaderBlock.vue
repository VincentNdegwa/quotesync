...
const daysLeft = computed(() => {
    if (!props.quote.validUntil) {
        return 0;
    }

    const diff = new Date(props.quote.validUntil).getTime() - Date.now();
    ...
    >
        <div v-if="config.layout === 'logo-left-details-right'" class="flex items-start justify-between gap-6">
            <div class="flex flex-col gap-2">
                <img
                    v-if="config.showLogo && branding.logoUrl"
                    :src="branding.logoUrl"
                    alt="Company logo"
                    class="h-12 w-auto object-contain"
                />
                <span v-if="!branding.logoUrl || !config.showLogo" class="text-lg font-bold" :style="{ color: branding.primaryColor }">
            ...
            <div class="space-y-1 text-right text-sm">
                ...
                <div v-if="config.showIssueDate" class="text-gray-500">Issued: {{ formatDate(quote.createdAt) }}</div>
                <div v-if="config.showValidUntil" class="text-gray-500">Valid until: {{ formatDate(quote.validUntil) }}</div>
                <div
                    v-if="config.showExpiryCountdown && daysLeft > 0"
                    class="inline-flex items-center gap-1 rounded-full px-2 py-0.5 text-xs font-medium"
                    :style="{
                        backgroundColor: daysLeft <= 7 ? '#FEF3C7' : `color-mix(in oklab, ${branding.primaryColor} 10%, white)`,
                        color: daysLeft <= 7 ? '#92400E' : branding.primaryColor,
                    }"
                >
        ...
        <div v-else-if="config.layout === 'logo-right-details-left'" class="flex items-start justify-between gap-6">
            ...
            <div>
                <img
                    ...
                    class="h-12 w-auto object-contain"
                />
                <span v-else class="text-lg font-bold" :style="{ color: branding.primaryColor }">{{ branding.companyName }}</span>
            </div>
        </div>
        ...
        <div v-else class="flex items-center gap-3">
            <span class="font-semibold" :style="{ color: branding.primaryColor }">{{ branding.companyName }}</span>
