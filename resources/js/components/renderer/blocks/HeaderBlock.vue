...
import type { HeaderBlockConfig, DocumentData, WorkspaceSettings } from '@/types';
    ...
    <div class="header-block" :style="blockBaseStyle(config)" :class="blockFontSizeClass(config.fontSize)">
        <div v-if="config.layout === 'logo-left-details-right'" class="flex items-start justify-between gap-6">
            ...
            <div class="space-y-1 text-right text-sm">
                <div v-if="config.showQuoteNumber" class="text-base font-semibold">{{ documentNumber ?? 'Draft' }}</div>
                <div v-if="config.showIssueDate" class="text-gray-500">Issued: {{ formatDate(issueDate) }}</div>
                <div v-if="config.showValidUntil" class="text-gray-500">{{ isQuote ? 'Valid until' : 'Due' }}: {{ formatDate(expiryDate) }}</div>
                <div
                ...
                >
                    {{ isQuote ? 'Expires' : 'Due' }} in {{ daysLeft }} day{{ daysLeft === 1 ? '' : 's' }}
                </div>
            </div>
        </div>
        ...
        <div v-else class="flex items-center gap-3">
            ...
            <span v-if="config.showQuoteNumber" class="text-sm text-gray-500">· {{ documentNumber ?? 'Draft' }}</span>
        </div>
    </div>
</template>
