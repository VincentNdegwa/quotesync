...
const emit = defineEmits<{
    ...
    (e: 'quick-add-line-item', payload: { sectionIndex: number; catalogItem: BuilderCatalogItem | null }): void;
}>();
...
const sections = computed<Section[]>(() => {
    ...
    const data = props.data as InvoiceData & { line_items?: DocumentLineItem[] };
...
const fontSizeSetting = computed<'sm' | 'md' | 'lg'>(() => {
    const configWithFont = props.config as LineItemsBlockConfig & { fontSize?: 'sm' | 'md' | 'lg' };
...
const availableCatalogItems = computed<BuilderCatalogItem[]>(() => props.catalogItems ?? []);
        ...
        <div v-for="(section, sectionIndex) in sections" :key="`section-${section.id ?? sectionIndex}`" class="group/section mb-8 last:mb-0">
            <div class="mb-3 flex items-center justify-between gap-2">
                ...
                <Button
                    ...
                    variant="ghost"
                    size="sm"
                    class="h-7 text-xs text-destructive opacity-0 transition-opacity group-hover/section:opacity-100 hover:text-destructive hover:bg-destructive/10"
                    ...
                    <Table class="table-fixed">
                        ...
                        <TableBody>
                            <template v-for="(item, lineItemIndex) in section.line_items" :key="`item-${item.id ?? lineItemIndex}`">
                                <TableRow
                                    :class="[stripeClass(lineItemIndex), editMode ? 'group/row cursor-pointer hover:bg-primary/5' : '']"
                                    :style="{ borderColor: config.border.color ?? undefined }"
                                ...
                                >
                                    <TableCell class="pr-4 align-top" :class="[cellPad, borderedCellClass, isGreyed(item) ? 'opacity-50' : '']" :style="{ borderColor: config.border.color ?? undefined }">
                                        <div class="flex items-start justify-between gap-3">
                                            <div class="min-w-0">
                                                <p class="font-medium" :class="titleClass">{{ item.name || 'Line item' }}</p>
                                                <p v-if="config.showItemDescription && item.description" class="mt-0.5 text-xs text-muted-foreground">
                                                    {{ item.description }}
                                                </p>
                                                <p v-else-if="config.showItemDescription && editMode" class="mt-0.5 text-xs text-muted-foreground/70">
                                                    Add description
                                                </p>
                                                <p v-if="config.showSku && item.catalog_item?.sku" class="mt-0.5 text-[10px] text-muted-foreground/70">
                                                    SKU {{ item.catalog_item?.sku }}
                                                </p>
                                            </div>
                                            <span
                                                v-if="editMode"
                                                class="text-[10px] uppercase tracking-wide text-primary opacity-0 transition group-hover/row:opacity-100"
                                            >
                                                Edit
                                            </span>
                                        </div>
                                        <span v-if="showBadge(item)" class="mt-1 inline-flex rounded-full border px-1.5 py-px text-[10px] uppercase tracking-wide text-muted-foreground">Optional</span>
                                    </TableCell>
                                    ...
                                    <TableCell v-if="showUnitPrice" class="text-right tabular-nums" :class="[cellPad, borderedCellClass, isGreyed(item) ? 'opacity-50' : '']" :style="{ borderColor: config.border.color ?? undefined }">
                                        <span>{{ fmt(itemUnitPrice(item), effectiveCurrency) }}</span>
                                    </TableCell>
                                    <TableCell v-if="config.showDiscount" class="text-right tabular-nums" :class="[cellPad, borderedCellClass, isGreyed(item) ? 'opacity-50' : '']" :style="{ borderColor: config.border.color ?? undefined }">
                                        <span>{{ item.discount_percent ? `${item.discount_percent}%` : '—' }}</span>
                                    </TableCell>
            ...
            <div v-if="editMode" class="mt-3">
                <div v-if="hasCatalogItems" class="inline-block" @click.stop>
                    <CatalogSearchPopover
                        ...
                        @select="(catalogItem) => emit('quick-add-line-item', { sectionIndex, catalogItem })"
                        @add-custom="emit('quick-add-line-item', { sectionIndex, catalogItem: null })"
                    >
                        <template #trigger>
                ...
                <Button
                    ...
                    size="sm"
                    class="h-7 text-xs"
                    @click.stop="emit('quick-add-line-item', { sectionIndex, catalogItem: null })"
                >
                    <Plus class="mr-1 h-3 w-3" />
                    Add item
                </Button>
