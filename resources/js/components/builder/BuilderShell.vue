...
import type { QuoteBuilderState, WorkspaceSettings, Block } from '@/types';
...
const { selectedBlockId, selectedBlock, blocks, sections } =
    ...
    builderStore.sections.forEach((section) => {
        section.line_items.forEach((lineItem) => {
            if (
                ...
                lineItem.price_tier_applied &&
                !lineItem.applied_price_tiers.length
            ) {
                const catalogItem = catalogItems.value.find(
                    (c) => c.id === lineItem.catalog_item_id,
                );
...
const handleInspectorUpdateBlock = (updatedBlock: Block | null): void => {
    if (updatedBlock) {
        builderStore.updateBlockConfig(
            String(updatedBlock.id),
            updatedBlock.config,
        );
    }
};
...
<template>
    <div class="flex h-full min-h-0 flex-col">
        ...
        <div class="grid min-h-0 flex-1 grid-cols-12 gap-4">
            <div
                class="col-span-2 min-h-0 overflow-y-auto rounded-lg border bg-card p-3"
            >
                <BlockList
                    :blocks="blocks"
