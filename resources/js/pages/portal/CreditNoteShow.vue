...
<template>
    ...
    <div class="space-y-6">
        <div class="flex items-center gap-4">
            ...
            <h1 class="text-2xl font-bold">{{ creditNote.credit_note_number }}</h1>
        </div>
                ...
                <div class="rounded-xl border bg-white p-6 shadow-sm">
                    ...
                    <div class="grid gap-4 text-sm">
                        ...
                        <div class="flex justify-between">
                            ...
                            <span class="font-medium">{{ formatDate(creditNote.issue_date) }}</span>
                        </div>
                        <div v-if="creditNote.due_date" class="flex justify-between">
                            <span class="text-muted-foreground">Due Date</span>
                            <span class="font-medium">{{ formatDate(creditNote.due_date) }}</span>
                        </div>
                        <div v-if="creditNote.applied_at" class="flex justify-between">
                            <span class="text-muted-foreground">Applied At</span>
                            <span class="font-medium">{{ formatDate(creditNote.applied_at) }}</span>
                        </div>
                    ...
                    <div>
                        ...
                        <p class="text-sm text-muted-foreground">{{ creditNote.reason }}</p>
                    </div>
                </div>
                ...
                <div class="rounded-xl border bg-white p-6 shadow-sm">
                    ...
                    <div class="space-y-3">
                        ...
                        >
                            ...
                            <div class="col-span-2 text-right">
                                {{ creditNote.currency }} {{ formatCurrency(item.unit_price) }}
                            </div>
                            <div class="col-span-2 text-right font-medium">
                                {{ creditNote.currency }} {{ formatCurrency(item.total) }}
                            </div>
                        </div>
                    </div>
                </div>
            ...
            <div class="space-y-6">
                <div class="rounded-xl border bg-white p-6 shadow-sm">
                    ...
                    <div class="space-y-2 text-sm">
                        <div class="font-medium">{{ creditNote.workspace.name }}</div>
                        <div class="text-muted-foreground">{{ creditNote.workspace.email }}</div>
                    </div>
                </div>
                ...
                <div class="rounded-xl border bg-white p-6 shadow-sm">
                    ...
                    <div class="space-y-2">
                        <div class="flex justify-between text-sm">
                            ...
                            <span>{{ creditNote.currency }} {{ formatCurrency(creditNote.amount) }}</span>
                        </div>
                        <div class="flex justify-between text-sm">
                            <span class="text-muted-foreground">Tax</span>
                            <span>{{ creditNote.currency }} {{ formatCurrency(creditNote.tax_amount) }}</span>
                        </div>
                        <Separator />
                        <div class="flex justify-between font-semibold">
                            <span>Total Credit</span>
                            <span>{{ creditNote.currency }} {{ formatCurrency(creditNote.total) }}</span>
                        </div>
                    </div>
                </div>
            </div>
