...
export const getCatalogColumns = (options: CatalogColumnOptions): ColumnDef<CatalogItemRecord>[] => [
    {
        id: 'select',
        enableSorting: false,
        enableHiding: false,
    ...
    {
        id: 'margin_percent',
        header: ({ column }) => h('div', { class: 'text-center' }, sortableHeader('Margin %', column, 'right')),
        accessorFn: (row) => options.marginPercent(row),
        cell: ({ row }) => h('div', { class: 'text-center tabular-nums' }, `${options.marginPercent(row.original)}%`),
    },
    {
        accessorKey: 'usage_count',
        header: ({ column }) => h('div', { class: 'text-center' }, sortableHeader('Usage', column, 'right')),
        cell: ({ row }) => h('div', { class: 'text-center tabular-nums' }, row.original.usage_count),
    },
    {
    ...
    },
    {
        ...
        cell: ({ row }) => h('div', { class: 'text-right' }, h(DataTableRowActions, {
            item: row.original,
