...
const templatePerformance = computed(() =>
    props.quote_performance.by_template.slice(0, 4),
);
const dealSizePerformance = computed(
    () => props.quote_performance.by_deal_size,
);
...
const maxCurrencyPipeline = computed(() =>
    currencyLeaders.value.length > 0
        ? Math.max(
              ...currencyLeaders.value.map((currency) => currency.pipeline),
          )
        : 0,
);
    ...
    <div class="space-y-6">
            ...
            >
                <div class="space-y-2">
                    ...
                    <CardTitle class="text-2xl font-semibold tracking-tight"
                        >Workspace analytics</CardTitle
                    >
                    <CardDescription
                        >Compare revenue momentum, loss drivers, and performance
                        signals in one place.</CardDescription
                    >
                </div>
            ...
            <CardFooter class="justify-between text-xs text-muted-foreground">
                ...
                <div class="flex items-center gap-4">
                    <span class="flex items-center gap-1.5">
                        ...
                        Figures shown in base currency. Snapshot rates applied
                        at creation.
                    </span>
                    <span
                        >Open pipeline:
                        {{ formatCurrency(forecast.open_pipeline) }}</span
                    >
                </div>
            </CardFooter>
        </Card>
        ...
        <section class="grid gap-4 xl:grid-cols-[1.7fr_1fr]">
            <Card class="border border-sidebar-border/70">
                <CardHeader class="pb-0">
                    ...
                    >
                    <CardDescription
                        >Track how won revenue compares to the rolling
                        three-month average.</CardDescription
                    >
                </CardHeader>
                        ...
                        >
                            ...
                            <ChartCrosshair
                                ...
                                :color="[
                                    ...
                                    revenueChartConfig.average.color,
                                ]"
                            />
                        </VisXYContainer>
            ...
            <Card class="border border-sidebar-border/70">
                <CardHeader class="pb-3">
                    <CardTitle class="text-base font-semibold"
                        >Revenue highlights</CardTitle
                    >
                    <CardDescription
                        >Recent monthly performance and average
                        movement.</CardDescription
                    >
                </CardHeader>
                <CardContent>
                    <div
                        v-if="revenueTimeline.length === 0"
                    ...
                    <ul v-else class="space-y-4">
                        <li
                            v-for="entry in revenueTimeline"
                            :key="entry.id"
                        ...
                        >
                            ...
                            <p class="text-xs text-muted-foreground">
                                Won {{ entry.value }} • Avg {{ entry.average }}
                            </p>
                            <p
                            ...
                            >
                                {{ entry.delta >= 0 ? '↑' : '↓' }}
                                {{ formatTrendValue(entry.delta) }}% vs prior
                                month
                            </p>
                        </li>
                    </ul>
        ...
        <section class="grid gap-4 xl:grid-cols-[1.4fr_1fr]">
            <Card class="border border-sidebar-border/70">
                <CardHeader class="pb-0">
                    <CardTitle class="text-base font-semibold"
                        >Time to win</CardTitle
                    >
                    <CardDescription
                        >Speed buckets for deals that moved to a closed
                        outcome.</CardDescription
                    >
                </CardHeader>
                <CardContent class="h-[320px]">
                    <div
                        v-if="timeToWinChartData.length === 0"
            ...
            <Card class="border border-sidebar-border/70">
                <CardHeader class="pb-3">
                    <CardTitle class="text-base font-semibold"
                        >Loss signals</CardTitle
                    >
                    <CardDescription
                        >Where deals fell through and impacted
                        revenue.</CardDescription
                    >
                </CardHeader>
                <CardContent>
                    <div
                        v-if="declineTimeline.length === 0"
                        class="rounded-lg border border-dashed p-4 text-sm text-muted-foreground"
                    >
                    ...
                    <ul v-else class="space-y-4">
                        ...
                        >
                            ...
                            <p class="text-xs text-muted-foreground">
                                {{ item.count }} deals • {{ item.value }} at
                                risk
                            </p>
                        </li>
                    </ul>
        ...
        <section class="grid gap-4 lg:grid-cols-3">
            <Card class="border border-sidebar-border/70">
                <CardHeader class="pb-3">
                    <CardTitle class="text-base font-semibold"
                        >Template performance</CardTitle
                    >
                    <CardDescription
                        >Top templates ranked by win rate.</CardDescription
                    >
                </CardHeader>
                <CardContent>
                    <div
                        v-if="templatePerformance.length === 0"
                        class="rounded-lg border border-dashed p-4 text-sm text-muted-foreground"
                    >
                    ...
                    <div v-else class="space-y-3">
                        <div
                            v-for="template in templatePerformance"
                        ...
                        >
                            ...
                            >
                                ...
                                <span class="font-medium">{{
                                    formatPercent(template.win_rate)
                                }}</span>
                            </div>
                            ...
                            <p class="text-xs text-muted-foreground">
                                {{ template.total_quotes }} quotes • Avg
                                {{ formatCurrency(template.avg_value) }}
                            </p>
                        </div>
                    </div>
                </CardContent>
            ...
            <Card class="border border-sidebar-border/70">
                <CardHeader class="pb-3">
                    <CardTitle class="text-base font-semibold"
                        >Deal size response</CardTitle
                    >
                    <CardDescription
                        >Win rate based on invoice value
                        buckets.</CardDescription
                    >
                </CardHeader>
                <CardContent>
                    <div
                        v-if="dealSizePerformance.length === 0"
                        class="rounded-lg border border-dashed p-4 text-sm text-muted-foreground"
                    >
                    ...
                    <div v-else class="space-y-3">
                        <div
                            v-for="bucket in dealSizePerformance"
                            :key="bucket.range"
                            class="space-y-1"
                        >
                            <div
            ...
            <Card class="border border-sidebar-border/70">
                <CardHeader class="pb-3">
                    <CardTitle class="text-base font-semibold"
                        >Discount impact</CardTitle
                    >
                    <CardDescription
                        >How price adjustments influence wins.</CardDescription
                    >
                </CardHeader>
                <CardContent>
                    <div
                        v-if="discountPerformance.length === 0"
                        class="rounded-lg border border-dashed p-4 text-sm text-muted-foreground"
                    >
                        Discount data will surface once quotes include
                        discounts.
                    </div>
                    <div v-else class="space-y-3">
                        <div
                            v-for="bucket in discountPerformance"
                            :key="bucket.range"
                            class="space-y-1"
                        >
                            <div
        ...
        <section class="grid gap-4 xl:grid-cols-[1.4fr_1fr]">
            <Card class="border border-sidebar-border/70">
                <CardHeader class="pb-3">
                    <CardTitle class="text-base font-semibold"
                        >Top clients</CardTitle
                    >
                    <CardDescription
                        >Clients ranked by total won revenue and
                        responsiveness.</CardDescription
                    >
                </CardHeader>
                <CardContent>
                    <div
                    ...
                    <div v-else class="space-y-3">
                        ...
                        >
                            ...
                            >
                                ...
                                <span
                                    >Avg response
                                    {{ client.avg_response_days }}d</span
                                >
                                <span class="font-medium text-foreground">{{
                                    formatCurrency(client.total_won)
                                }}</span>
                            </div>
                        </div>
                    </div>
            ...
            <Card class="border border-sidebar-border/70">
                <CardHeader class="pb-3">
                    <CardTitle class="text-base font-semibold"
                        >Currencies in play</CardTitle
                    >
                    <CardDescription
                        >Where pipeline is accumulating by billing
                        currency.</CardDescription
                    >
                </CardHeader>
                <CardContent>
                    <div
                        v-if="currencyLeaders.length === 0"
                        class="rounded-lg border border-dashed p-4 text-sm text-muted-foreground"
                    >
                    ...
                    <ul v-else class="space-y-4">
                        <li
                            v-for="currency in currencyTimeline"
                            :key="currency.id"
                            class="relative pl-10"
                        >
                            <div
                            ...
                            <div class="flex items-center justify-between">
                                ...
                                <Badge variant="outline"
                                    >{{ currency.quotes }} quotes</Badge
                                >
                            </div>
                            <p class="text-xs text-muted-foreground">
                                Pipeline {{ currency.pipeline }} • Won
                                {{ currency.won }}
                            </p>
                            <div class="mt-2 h-2 w-full rounded-full bg-muted">
                                <div
                                    ...
                                    :style="{
                                        width:
                                            maxCurrencyPipeline > 0
                                                ? `${Math.max(4, (currency.pipelineRaw / maxCurrencyPipeline) * 100)}%`
                                                : '4%',
                                    }"
                                />
                            </div>
        ...
        <Card class="border border-sidebar-border/70">
            <CardHeader class="pb-3">
                <CardTitle class="text-base font-semibold"
                    >Forecast outlook</CardTitle
                >
                <CardDescription
                    >Projected revenue scenarios from the current
                    pipeline.</CardDescription
                >
            </CardHeader>
            <CardContent>
