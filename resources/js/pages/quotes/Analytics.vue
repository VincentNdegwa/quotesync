...
type AnalyticsData = {
  ...
  section_engagement: Array<{ section: string; time_spent_seconds: number; count: number }>;
...
const getTimelineIcon = (icon: string): Component => timelineIconMap[icon as TimelineIconKey] ?? MessageSquare;
...
const longestSession = computed(() => {
  ...
  return props.analytics.view_timeline.reduce((max, view) => {
    return (view.duration_seconds ?? 0) > (max.duration_seconds ?? 0) ? view : max;
...
const statCards = computed(() => [
  {
    ...
    sublabel: props.analytics.opened_count === 1 ? 'view recorded' : 'views recorded',
    trend: null,
  },
  {
    key: 'time',
  ...
  },
  {
    key: 'last-opened',
    title: 'Last opened',
    value: props.analytics.last_opened_at ? formatRelativeTime(props.analytics.last_opened_at) : 'Never',
    sublabel: props.analytics.last_opened_at ? formatDateTime(props.analytics.last_opened_at) : 'No open events',
    trend: null,
  },
]);
  ...
  <div class="space-y-6">
    <Card class="border border-sidebar-border/70">
      <CardHeader class="gap-4">
        <div class="flex flex-col gap-6 md:flex-row md:items-start md:justify-between">
          ...
          <div class="flex flex-wrap items-center gap-2 md:justify-end">
            <QuoteActions :quote="quote" :quote-statuses="quoteStatuses" variant="buttons" />
      ...
      <CardFooter class="justify-between text-xs text-muted-foreground">
        ...
        <span v-if="quote.valid_until">Valid until {{ formatDateTime(quote.valid_until) }}</span>
      </CardFooter>
    </Card>

    <section class="grid gap-4 md:grid-cols-3">
      <Card v-for="card in statCards" :key="card.key" class="border border-sidebar-border/70">
        <CardHeader class="space-y-2 pb-2">
          <CardDescription class="text-xs uppercase tracking-wide text-muted-foreground">{{ card.title }}</CardDescription>
          <div class="flex items-end justify-between gap-4">
            <CardTitle class="text-3xl font-semibold tracking-tight">{{ card.value }}</CardTitle>
            <div v-if="card.trend !== null" class="flex items-center gap-1 text-xs">
              <span :class="card.trend >= 0 ? 'text-emerald-600' : 'text-rose-600'" class="flex items-center gap-1 font-semibold">
        ...
        <CardFooter class="pt-0 text-xs text-muted-foreground">{{ card.sublabel }}</CardFooter>
      </Card>
    </section>
    ...
    <section class="grid gap-4 lg:grid-cols-[1.1fr_1fr]">
      <Card class="border border-sidebar-border/70">
        <CardHeader class="pb-0">
          ...
          <CardDescription>Device mix and engagement by channel.</CardDescription>
        </CardHeader>
        <CardContent class="space-y-4">
          <div v-if="deviceChartData.length === 0" class="rounded-lg border border-dashed p-4 text-sm text-muted-foreground">
          ...
          <template v-else>
            ...
            <div v-if="primaryDevice" class="rounded-xl border bg-muted/40 px-4 py-3 text-sm">
              <div class="flex items-center justify-between">
                <div class="flex items-center gap-3">
                  <div class="rounded-full bg-primary/10 p-2 text-primary">
                    <component :is="getDeviceIcon(primaryDevice.device)" class="h-4 w-4" />
                  </div>
                  <div>
                    <p class="font-medium capitalize">{{ getDeviceLabel(primaryDevice.device) }}</p>
                    <p class="text-xs text-muted-foreground">{{ primaryDevice.count }} views · {{ primaryDevice.percentage }}%</p>
                  </div>
                </div>
            ...
            <ul class="space-y-3 text-sm">
              <li v-for="device in deviceChartData" :key="device.device" class="space-y-1">
                <div class="flex items-center justify-between">
                  <div class="flex items-center gap-2 text-muted-foreground">
                    <component :is="getDeviceIcon(device.device)" class="h-4 w-4" />
                    <span class="capitalize">{{ getDeviceLabel(device.device) }}</span>
                  </div>
                  <span class="font-medium">{{ device.percentage }}%</span>
                </div>
                <div class="h-2 w-full rounded-full bg-muted">
                  <div class="h-2 rounded-full bg-primary" :style="{ width: `${device.percentage}%` }" />
                </div>
              </li>
            </ul>
          </template>
    ...
    <section class="grid gap-4 lg:grid-cols-[1.1fr_1fr]">
      <Card v-if="sectionChartData.length > 0" class="border border-sidebar-border/70">
        <CardHeader class="pb-0">
          <CardTitle class="text-base font-semibold">Reading depth</CardTitle>
          <CardDescription>Sections ranked by time spent.</CardDescription>
        </CardHeader>
        <CardContent class="h-[320px]">
          <ChartContainer :config="sectionChartConfig" cursor class="h-full">
            <VisXYContainer :data="sectionChartData" :margin="{ left: 140, right: 24, top: 24, bottom: 24 }">
              ...
              <VisAxis type="x" :num-ticks="4" :tick-line="false" :domain-line="false" :tick-format="(value: number) => `${Math.round(value)}s`" />
              <ChartTooltip />
              <ChartCrosshair
                :template="componentToString(sectionChartConfig, ChartTooltipContent, { labelKey: 'section', indicator: 'line' })"
                :color="[sectionChartConfig.seconds.color]"
              />
            </VisXYContainer>
            <ChartLegendContent class="mt-4 justify-start" />
        ...
        <CardFooter class="justify-between text-xs text-muted-foreground">
          <span>{{ sectionInsight ?? 'Engagement insights update live with every view.' }}</span>
      ...
      <Card class="border border-sidebar-border/70">
        <CardHeader class="pb-3">
          <CardTitle class="text-base font-semibold">Viewing timeline</CardTitle>
          <CardDescription>Every open, device, and duration in sequence.</CardDescription>
        </CardHeader>
        <CardContent>
          <div v-if="sessionTimeline.length === 0" class="rounded-lg border border-dashed p-4 text-sm text-muted-foreground">
            The engagement trail will appear once the quote is viewed.
          </div>
          <ul v-else class="space-y-4">
            <li v-for="entry in sessionTimeline" :key="entry.id" class="relative pl-10">
              ...
              <div class="flex items-center justify-between text-sm">
                ...
                <Badge variant="outline">{{ entry.durationLabel }}</Badge>
              </div>
              <p class="text-xs text-muted-foreground">{{ entry.timestamp }}</p>
              <p class="text-xs text-muted-foreground">Device · {{ entry.deviceLabel }}</p>
              <div v-if="entry.highlight" class="mt-2 inline-flex items-center gap-2 rounded-full border border-orange-200 bg-orange-50 px-3 py-1 text-xs font-medium text-orange-700 dark:border-orange-900/40 dark:bg-orange-950/30 dark:text-orange-300">
    ...
    <Card class="border border-sidebar-border/70">
      <CardHeader class="pb-3">
        <CardTitle class="text-base font-semibold">Communication log</CardTitle>
        <CardDescription>Follow-ups, deliveries, and outcomes for this quote.</CardDescription>
      </CardHeader>
      <CardContent>
        <div v-if="followUpTimeline.length === 0" class="rounded-lg border border-dashed p-4 text-sm text-muted-foreground">
          Communication history will appear after you engage with the client.
        </div>
        <ul v-else class="space-y-4">
          <li v-for="item in followUpTimeline" :key="item.id" class="relative pl-10">
            <div class="absolute left-0 top-2 flex h-full w-5 justify-center">
              <div class="h-full w-px bg-border" />
            </div>
            <div class="absolute -left-1 top-1 flex h-6 w-6 items-center justify-center rounded-full border bg-background">
            ...
            <p class="text-xs text-muted-foreground">{{ item.dateLabel }}</p>
          </li>
        </ul>
      </CardContent>
    </Card>
