...
const COLUMN_ORDER = ['draft', 'sent', 'partial', 'paid', 'overdue', 'void'] as const;
...
const COLUMN_STYLES: Record<StatusKey, ColStyle> = {
    ...
    partial:  { topBar: 'bg-amber-500',   countBg: 'bg-amber-50',    countText: 'text-amber-600',   dropActive: 'ring-2 ring-amber-400 bg-amber-50/60',     dot: 'bg-amber-500',   badge: 'bg-amber-50',    badgeText: 'text-amber-600' },
    paid:     { topBar: 'bg-emerald-500', countBg: 'bg-emerald-50',  countText: 'text-emerald-700', dropActive: 'ring-2 ring-emerald-400 bg-emerald-50/60', dot: 'bg-emerald-500', badge: 'bg-emerald-50',  badgeText: 'text-emerald-700' },
    overdue:  { topBar: 'bg-rose-500',    countBg: 'bg-rose-50',     countText: 'text-rose-600',    dropActive: 'ring-2 ring-rose-400 bg-rose-50/60',       dot: 'bg-rose-500',    badge: 'bg-rose-50',     badgeText: 'text-rose-600' },
    void:     { topBar: 'bg-gray-500',    countBg: 'bg-gray-50',     countText: 'text-gray-600',    dropActive: 'ring-2 ring-gray-400 bg-gray-50/60',       dot: 'bg-gray-500',    badge: 'bg-gray-50',     badgeText: 'text-gray-600' },
};

const columns = computed(() =>
    COLUMN_ORDER.map((status) => ({
        status,
        label: props.invoiceStatuses.find((s) => s.value === status)?.label ?? status,
...
const dragging = ref<{ invoice: InvoiceListRecord; fromStatus: string } | null>(null);
    ...
    <div v-else class="w-full overflow-x-auto custom-scrollbar pb-4">
        <div class="flex min-w-max gap-3 px-0.5 pt-0.5">
            ...
            >
                ...
                >
                    <div
                        v-if="col.invoices.length === 0 && dragging && canDrop(col.status)"
                    ...
                    <div
                        ...
                        :class="[
                            ...
                            dragging?.invoice.id === invoice.id ? 'opacity-40 scale-95' : '',
                        ]"
                    ...
                    >
                        ...
                        <div v-if="invoice.client" class="mb-1.5 flex items-center gap-1.5 text-xs text-muted-foreground">
                            <User class="h-3 w-3 shrink-0" />
                            <span class="truncate">{{ invoice.client.company_name }}</span>
                        </div>
                        ...
                        <div class="flex items-center justify-between gap-2 border-t border-border/50 pt-1.5">
                            <span class="text-xs font-semibold tabular-nums text-foreground">
                                {{ fmt(invoice.base_total, invoice.base_currency) }}
                            </span>
                            <div v-if="invoice.due_date" class="flex items-center gap-1 text-xs text-muted-foreground">
                        ...
                        <Transition name="hint-slide">
                            ...
                            >
                                <div v-if="isTerminal(invoice.status as StatusKey)" class="text-[10px] italic text-muted-foreground">
                                    Final status — cannot be moved
                                </div>
                                <div v-else class="flex flex-wrap items-center gap-1">
                                    <span class="mr-0.5 text-[10px] leading-none text-muted-foreground">Move to:</span>
                                    <span
                                        v-for="target in validTargets(invoice.status as StatusKey)"
                                        :key="target"
